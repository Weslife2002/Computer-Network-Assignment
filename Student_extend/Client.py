...
class Client:
    ...
    def describe(self):
        if self.state != self.INIT:
            self.rtspSeq += 1
            reply = "DESCRIBE " + self.fileName + " RTSP/1.0\nCSeq: " + str(self.rtspSeq)
            reply += "\nSession: " + str(self.sessionId)
            reply = reply.encode('utf-8')
            self.rtspSocket.send(reply)
        else:
            print("[DESCRIBE ERROR] Something went wrong! Please try again!")
    ...
    def exitClient(self):
        if self.state != self.INIT:
            ...
            if not self.serverInfo['event'].isSet():
    ...
    def playMovie(self):
        if self.state == self.INIT or self.state == self.PAUSE:
            if self.state == self.INIT:
                self.setupMovie()
